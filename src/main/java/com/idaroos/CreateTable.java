...
public void createTables() throws SQLException {
    ...
    try (Connection connection = databaseConnector.getConnection();
        ...
        String transactionQuery = "CREATE TABLE IF NOT EXISTS transactions (id INT PRIMARY KEY AUTO_INCREMENT, tocustomer_id INT, fromcustomer_id INT, amount INT, created DATETIME DEFAULT CURRENT_TIMESTAMP);";

        int cResult = statement.executeUpdate(customerQuery);
        int aResult = statement.executeUpdate(accountQuery);
        int tResult = statement.executeUpdate(transactionQuery);
        System.out.println("Result: " + cResult + aResult + tResult);
    }
